...
for mass in mass_points_low + mass_points_high:
    ...
    # prepare folders
    folder_path_stats = pathlib.Path(f"{region}_stats_finer_scan/{mass_str}")
    folder_path_stats.mkdir(parents=True, exist_ok=True)
    folder_path = pathlib.Path(f"{region}_sys_finer_scan/{mass_str}")
...
copyfile(f"./{fit_script_name}", pathlib.Path(f"low_mass_stats_finer_scan").joinpath(fit_script_name))
copyfile(f"./{fit_script_name}", pathlib.Path(f"low_mass_sys_finer_scan").joinpath(fit_script_name))
copyfile(f"./{fit_script_name}", pathlib.Path(f"high_mass_stats_finer_scan").joinpath(fit_script_name))
copyfile(f"./{fit_script_name}", pathlib.Path(f"high_mass_sys_finer_scan").joinpath(fit_script_name))
